...
void Calendario::revisarNacimiento() {
    ...
    if (inFile.is_open()) {
        ...
        cout << "Primera vez interactuando con Corazoncito. Creándolo..." << endl;
        cout << "¿Cómo lo quieres nombrar?" << endl;
        getline(cin, nombre);
        tiempoActual = time(0);
        struct tm datetime;
...
void Logro::completarLogro() const {
    ...
    cout << "Fecha de obtención: " << fechaDeObtencion << endl;
...
int Meta::marcarMeta() {  // ✅ Implementation in `Meta.cpp`
    if (progresoMeta >= cantidadMeta) {
        ...
        std::cout << "Sigue así, pronto completarás la Meta." << std::endl;
        metaCumplida = false;
        return 2;
    }
}
...
ConsecuenciaNegativa::ConsecuenciaNegativa() {
    frases = { "Estoy triste...", "Me siento débil...", "Por favor... ya no más..." };
}
...
void Corazoncito::crearHabito(string habitoTemp) {
    if (habitosGuardados < 3) {
        ...
        cout << "Máximo de hábitos alcanzado." << endl;
    }
}
...
void Corazoncito::ImprimirHabitos() {
    for (int i = 0; i < habitosGuardados; i++) {
        if (habitosCreados[i]) {  //Verifica si el puntero no es nulo antes de acceder
            cout << "Hábito: " << habitosCreados[i]->getNombre() << std::endl;
        }
        else {
...
void Corazoncito::revisarHabitosHechos() {
    cout << "Marca si has cumplido tus hábitos de hoy! :D" << endl;
    ...
    for (int i = 0; i < habitosGuardados; i++) {
        ...
        do {
            if (respuesta == 's' || respuesta == 'S') {
                ...
                cout << "Error! Letra inválida! o_O Pon 's' o 'n'" << endl;
            }
        } while (true);
    }
...
void Corazoncito::crearMeta() {
    ...
    cout << "Elige la meta que deseas realizar" << endl;
    cin.ignore();
    getline(cin, meta);

    cout << "¿Cuánto deseas realizar de la meta?" << endl;
    cin >> cantidad;
